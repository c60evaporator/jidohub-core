...
命名は ``<入力>_to_<出力>`` のスキームで統一する。これにより
...
class TaskType(str, Enum):
    ...
    値は kebab-case の文字列。JSON への直列化時はそのまま文字列になる
    （``str`` を継承しているため）。
    """

    # --- 知覚系（単体タスク） -------------------------------------------
    SENSING_TO_DETECTION = "sensing-to-detection"
    """センサ入力 → 3D 物体検出。出力は ``Detection3DOutput``。

    例: CenterPoint, TransFusion, BEVFusion
    """

    SENSING_TO_TRACK = "sensing-to-track"
    """センサ入力 → 3D 物体追跡。出力は ``Detection3DOutput``（``track_id`` 付き）。"""

    SENSING_TO_MAP = "sensing-to-map"
    ...
    # --- 統合系 ---------------------------------------------------------
    SENSING_TO_PLANNING = "sensing-to-planning"
    ...
    SENSING_TO_PLANNING_TEXT = "sensing-to-planning-text"
    """センサ入力 → 走行軌跡 + 自然言語（VLA）。出力は ``VLAOutput``（将来定義）。

    Phase 1 では出力スキーマ未定義。enum の値のみ予約する。
    """

    TRACK_MAP_TO_PLANNING = "track-map-to-planning"
    """追跡結果 + マップ → 走行軌跡（プランナ単体）。出力は ``PlanningOutput``。"""

    PLANNING_TO_CONTROL = "planning-to-control"
